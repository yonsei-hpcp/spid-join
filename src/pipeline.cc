...
namespace pidjoin
{
    void JoinInstance::AddJobToEndMap(rotate_n_stream_job_t *ended_job)
    {
        ...
        if (ended_job->num_rank_for_multi_rank_copy != 0)
        {
            for (int r = 0; r < ended_job->num_rank_for_multi_rank_copy; r++)
            {
                bool status_dst = this->UpdateJobStatus(ended_job->rank_ids_for_multi_rank_copy[r]);
                if (status_dst)
                {
                    this->WakeUpQueueLine(ended_job->rank_ids_for_multi_rank_copy[r]);
                }
        ...
        if (ended_job->num_rank_for_multi_rank_copy != 0)
        {
            dpuhandler->rnc_job_q->ROTATE_AND_STREAM_JOBS-= ended_job->num_rank_for_multi_rank_copy;
        }
        else
        {
        ...
        DPUKernelParams_t *param)
    {
        ...
        for (; progress < curr_query_tree->size(); progress += 1)
        {
            ...
            case DPU_FUNC_LOCAL_HASH_PARTITIONING:
                join_operator->Execute_LOCAL_HASH_PARTITIONING(this, rank_id, param, &query_node);
                break;
            ...
            case DPU_FUNC_PHJ_PROBE_HASH_TABLE_INNER_LINEAR_PROBE:
                ...
                break;
            case DPU_FUNC_MPSM_JOIN_LOCAL_PARTITIONING:
                join_operator->Execute_MPSM_JOIN_LOCAL_PARTITIONING(this, rank_id, param, &query_node);
                break;
            case DPU_FUNC_MPSM_JOIN_SORT_PROBE:
                join_operator->Execute_MPSM_JOIN_SORT_PROBE(this, rank_id, param, &query_node);
                break;
            /////////////////////////////
            // Host Functions
            /////////////////////////////
            ...
            case HOST_FUNC_CALCULATE_PAGE_HISTOGRAM:
                ...
                break;
            case HOST_FUNC_CALCULATE_BANKGROUP_HISTOGRAM:
                join_operator->Execute_MT_SAFE_HOST_FUNC_CALCULATE_BANKGROUP_HISTOGRAM(this, rank_id, param, &query_node);
                break;
            ...
            case HOST_FUNC_RECV_DATA_OPT:
                ...
                break;
            case HOST_FUNC_BROADCAST_DATA:
                join_operator->Execute_HOST_FUNC_BROADCAST_DATA(this, rank_id, param, &query_node);
                break;
            case HOST_FUNC_CHIPWISE_BROADCAST:
                join_operator->Execute_HOST_FUNC_CHIPWISE_BROADCAST(this, rank_id, param, &query_node);
                break;
            /////////////////////////////
            // Control Functions
            /////////////////////////////
            ...
            case COMPOUND_FUNC_RNS_JOIN:
                ...
                break;
            case COMPOUND_FUNC_IDP_JOIN:
                join_operator->Execute_COMPOUND_FUNC_IDP_JOIN(this, rank_id, param, &query_node, &perThreadTimeStamps_detail);
                break;
            case COMPOUND_FUNC_GLB_PARTITION:
                join_operator->Execute_COMPOUND_FUNC_GLB_PARTITION(this, rank_id, param, &query_node);
                break;
            case COMPOUND_FUNC_GLB_CHIPWISE_PARTITION:
                join_operator->Execute_COMPOUND_FUNC_GLB_CHIPWISE_PARTITION(this, rank_id, param, &query_node);
                break;
            case COMPOUND_FUNC_BG_BROADCAST:
                join_operator->Execute_COMPOUND_FUNC_BG_BROADCAST(this, rank_id, param, &query_node);
                break;
    ...
    void *rank_pipeline_execution_thr_func(void *arg)
    {
        ...
        // Start proceeding query plans
        int ret = rank_thread_handle_arg->idp_handler->ProceedQueryPlan(
            curr_join_operator, rank_thread_handle_arg->rank_id, &params);
    ...
    void IDPHandler::SetQueryTree(join_design_t& jd)
    {
        this->join_design = jd;

        // To check bank set count
        if (!(jd.bank_set == 1 or jd.bank_set == 8 or jd.bank_set == 16 or jd.bank_set == 32 or jd.bank_set == 64))
        {
            printf("Error! jd.bank_set is set as %d. bank_set should be 1, 8, 16, 32, or 64.\n", jd.bank_set);
            exit(-1);
        }
        ...
        Json::Reader reader;
        Json::Value *r_handle = new Json::Value();
        Json::Value *s_handle = new Json::Value();
        ...
        assert(reader.parse(str, *r_handle));
        this->curr_query_tree = &((*r_handle)["query_tree"]);
    }

    void IDPHandler::AllocateRankPipeline(std::vector<int> &rank_ids, int number_of_ranks, join_design_t join_design)
    {
        this->AllocateRank(rank_ids, number_of_ranks);
        this->SetQueryTree(join_design);
        ...
        for (int r = 0; r < rank_ids.size(); r++)
        {
            ...
            this->rank_thread_handle[r].join_design = join_design;
