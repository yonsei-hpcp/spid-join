...
int main(void)
{
	...
	if (tasklet_id == 0)
	{
		...
		if (param_packetwise_local_hash_partitioning.do_calculate_partition_num != 0)
		{
			printf("\n\nRANK_ID: %d, DPU_ID: %d, TOTAL_ELEM_NUM: %d\n\n", 
				param_packetwise_local_hash_partitioning.rank_id,
				param_packetwise_local_hash_partitioning.dpu_id,
				TOTAL_ELEM_NUM);
		}
