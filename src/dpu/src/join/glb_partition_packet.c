...
int main(void)
{
    ...
    // if (param_glb_partition_packet.partition_type == GLB_PART_ARR_L0)
    {
        ...
        RadixPartitionArrayPacked(
            ...
            param_glb_partition_packet.hist_interval,
            param_glb_partition_packet.join_type);
    }
...
int RadixPartitionArrayPacked(
    ...
    int join_type)
{
    if (param_glb_partition_packet.partition_type == 0)
    {
        ...
        for (int byte_addr = tasklet_id * BLOCK_SIZE2; byte_addr < tot_bytes; byte_addr += (NR_TASKLETS * BLOCK_SIZE2))
        {
            ...
            for (int e = 0; e < elem_n; e++)
            {
                ...
                if (wram_buff[e].lvalue != 0)
                {
                    ...
                    uint32_t hash_val = 0;
                    hash_val = (RADIX & glb_partition_hash(wram_buff[e].lvalue));
    ...
    {
        int RADIX = (partition_num / param_glb_partition_packet.num_rankgroup) - 1;
        int rank_group_mask = param_glb_partition_packet.num_rankgroup - 1;
        int rank_group_multiplier = param_glb_partition_packet.num_rank_in_rankgroup * 64;
        ...
        for (int byte_addr = tasklet_id * BLOCK_SIZE2; byte_addr < tot_bytes; byte_addr += (NR_TASKLETS * BLOCK_SIZE2))
        {
            ...
            for (int e = 0; e < elem_n; e++)
            {
                if (wram_buff[e].lvalue != 0)
                {
                    ...
                    uint32_t hash_val = 0;
                    hash_val = (RADIX & glb_partition_hash(wram_buff[e].lvalue)) + ((e & rank_group_mask) * rank_group_multiplier);
