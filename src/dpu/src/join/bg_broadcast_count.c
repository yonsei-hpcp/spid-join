...
// Variables from Host
__host bg_broadcast_count_arg param_bg_broadcast_count;
    ...
    int num_partition)
{
    ...
    // int RADIX = ((num_partition - 1) << 3) & 0xfffffff8;
    ...
    while (1)
    {
        ...
        for (int i = 0; i < elem_size; i++)
        {
            if (wram_read_buffer_payload[i].lvalue != 0)
            {
                ...
                // uint32_t val = (RADIX & glb_partition_hash(wram_read_buffer_payload[i].lvalue)) >> 3;
        ...
        if (remains > 0)
        {
            if (remains == 4)
            {
                mram_write(
                    ...
                    (__mram_ptr void *) 
                    ((uint32_t)mram_partition_info_base_addr + (loops)*BLOCK_SIZE2), 8);
            }
            else
            {
                mram_write(
                    (void*)(write_buffer + (loops)*BLOCK_SIZE2), 
                    (__mram_ptr void *) 
                    ((uint32_t)mram_partition_info_base_addr + (loops)*BLOCK_SIZE2), remains);
            }
        }
    }
...
int main(void)
{
    ...
    if (tasklet_id == 0)
    {
        ...
        memset(write_buffer, 0, param_bg_broadcast_count.partition_num*sizeof(int32_t));
    }
    barrier_wait(&my_barrier);
    ...
    // Allocate Buffers for each tasklet
    read_buffer_payload = (tuplePair_t*)mem_alloc(BLOCK_SIZE2);

    HashPartitionCalculationArray(
        tasklet_id,
        MRAM_BASE_ADDR + param_bg_broadcast_count.input_offset, 
        MRAM_BASE_ADDR + param_bg_broadcast_count.partition_info_start_byte, 
        MRAM_BASE_ADDR + param_bg_broadcast_count.histogram_start_byte, 
        param_bg_broadcast_count.elem_num,
        read_buffer_payload,
        param_bg_broadcast_count.partition_num);
