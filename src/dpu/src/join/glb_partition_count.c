...
int HashPartitionCalculationArray(
    ...
    int num_partition)
{
    if (param_glb_partition_count.partition_type == 0)
    {
        ...
        while (1)
        {
            ...
            for (int i = 0; i < elem_size; i++)
            {
                ...
                {
                    uint32_t val = 0;
                    val = (RADIX & glb_partition_hash(wram_read_buffer_payload[i].lvalue));
    ...
    {
        int RADIX = (num_partition / param_glb_partition_count.num_rankgroup) - 1;
        int rank_group_mask = param_glb_partition_count.num_rankgroup - 1;
        int rank_group_multiplier = param_glb_partition_count.num_rank_in_rankgroup * 64;
        ...
        while (1)
        {
            ...
            for (int i = 0; i < elem_size; i++)
            {
                if (wram_read_buffer_payload[i].lvalue != 0)
                {
                    uint32_t val = 0;
                    val = (RADIX & glb_partition_hash(wram_read_buffer_payload[i].lvalue))
                            + (i & rank_group_mask) * rank_group_multiplier;
...
int main(void)
{
    ...
    HashPartitionCalculationArray(
        tasklet_id,
        param_glb_partition_count.join_type,
        ...
        param_glb_partition_count.hist_interval,
        read_buffer_payload,
