...
int main(void)
{
    ...
    if (tasklet_id == 0)
    {
        ...
        partition_count = param_glb_partition_packet.partition_num;
    }
...
int RadixPartitionArrayPacked(
    ...
    int packet_size)
{
    int RADIX = ((partition_num/param_glb_partition_packet.bankgroup)/param_glb_partition_packet.num_rankgroup) - 1;
    uint32_t TID_RADIX = (param_glb_partition_packet.bankgroup-1);
    uint32_t TID_multiplier = (log2(param_glb_partition_packet.bankgroup));
    int num_bankgroup_per_rank = (64 / param_glb_partition_packet.bankgroup);
    int num_bankgroup_per_rank_sq = log2(num_bankgroup_per_rank);
    int num_bankgroup_per_rank_RADIX = num_bankgroup_per_rank - 1;
    int bankgroup_RADIX = param_glb_partition_packet.bankgroup - 1;
    uint32_t shiftval = TID_multiplier - 3;
    uint32_t rankgroup_mask = param_glb_partition_packet.num_rankgroup - 1;
    uint32_t rankgroup_multiplier = param_glb_partition_packet.num_rank_in_rankgroup * 64;
    uint32_t rankgroup_sq = log2(rankgroup_multiplier);
    
    if (param_glb_partition_packet.bankgroup == 8)
    {
        LUT_BG = LUT_BG8;
    }
    else if (param_glb_partition_packet.bankgroup == 16)
    {
        LUT_BG = LUT_BG16;
    }
    else if (param_glb_partition_packet.bankgroup == 32)
    {
        LUT_BG = LUT_BG32;
    }
    else if (param_glb_partition_packet.bankgroup == 64)
    {
        LUT_BG = LUT_BG64;
    }
    ...
    int cntr = 1;
    int slot = BLOCK_SIZE2_ELEM / param_glb_partition_packet.num_rankgroup;
    ...
    for (int byte_addr = tasklet_id * BLOCK_SIZE2; byte_addr < tot_bytes; byte_addr += (NR_TASKLETS * BLOCK_SIZE2))
    {
        ...
        for (uint32_t e = 0; e < elem_n; e++)
        {
            ...
            if (wram_buff[e].lvalue != 0)
            {
                ...
                int temp = (RADIX & glb_partition_hash(wram_buff[e].lvalue));
                uint32_t hash_val = ((uint32_t)((temp&num_bankgroup_per_rank_RADIX)<<shiftval) 
                    + (uint32_t)((temp >> num_bankgroup_per_rank_sq) << 6)
                    + (uint32_t)(LUT_BG[e & bankgroup_RADIX]));
                hash_val +=  ((e / slot) & rankgroup_mask) << rankgroup_sq;
                

                // if (hash_val >= partition_num)
                // {
                //     dpu_results.ERROR_TYPE_0 = hash_val;
                // }
