...
namespace pidjoin
{
    ...
    void IDPMemoryMgr::RemoveNode(const char *col_name)
    {
        ...
        {
            printf("%sError2: nodename: %s\nthis->mram_space_idxs[curr_idx]:%d this->mram_space_idxs[curr_idx+1]:%d != start_byte + node->block_byte %d.\n",
    ...
    int IDPMemoryMgr::FindEmptySpace(StackNode *new_stacknode, int mram_align_size, int block_byte)
    {
        ...
        {
            PrintCurrentMramStatus();
            printf("Error %s No More Empty Space on MRAM for Node: %s\n", KRED, new_stacknode->name.c_str());
            exit(-1);
        }
        else
        {
...
#define MAN_RANK_NUM 16
    void IDPHandler::AllocateRank(std::vector<int> &rank_ids, int num)
    {
        ...
        for (int new_rank_id = 0; new_rank_id < (num) && all_rank_id < 40;)
        {
            ...
            int allocated_dpus = 0;
            DPU_ASSERT(dpu_alloc_custom(NUM_DPU_RANK, NULL, dpu_set, &allocated_dpus));
            
            if (channel_ids[all_rank_id] >= 9)
            {
                if (allocated_dpus == NUM_DPU_RANK)
                {
    ...
    void IDPHandler::HandleKernelResults(JoinInstance *join_instance, int rank_id, int DPU_FUNC_TYPE, DPUKernelParams_t &params, char *ptr)
    {
        ...
        break;
        case DPU_FUNC_PACKETWISE_NPHJ_PROBE:
        {
            packetwise_nphj_probe_return_arg* return_arg 
                = (packetwise_nphj_probe_return_arg*)malloc(param_return_size_map[DPU_FUNC_TYPE] * NUM_DPU_RANK);
            int i = 0;
            struct dpu_set_t dpu_set;
            ...
            packetwise_nphj_probe_arg *arg = (packetwise_nphj_probe_arg *)(params[0]);

            int tuple_size = sizeof(tuplePair_t);

            StackNode *result_s_node = this->FindNode(rank_id, arg->result_start_byte);
            ...
            for (int dpu = 0; dpu < NUM_DPU_RANK; dpu++)
            {
                ...
                S += (arg[dpu].num_input_bytes / sizeof(int64_t));
#ifdef COLLECT_LOGS
        ...
        case DPU_FUNC_PHJ_PROBE_HASH_TABLE_INNER_LINEAR_PROBE:
        {
            ...
            StackNode *result_s_node = this->FindNode(rank_id, arg->parted_S_offset);
        ...
        {
            sort_merge_probe_return_arg *return_arg = (sort_merge_probe_return_arg *)malloc(param_return_size_map[DPU_FUNC_TYPE] * NUM_DPU_RANK);
            ...
            // Should be fixed later
            StackNode *result_s_node = this->FindNode(rank_id, arg->r_sorted_start_byte);
        ...
        // //     sort_merge_sort_probe_arg *arg = (sort_merge_sort_probe_arg *)(params[0]);

        // //     int tuple_size = sizeof(tuplePair_t);

        // //     // Should be fixed later
        // //     StackNode *result_s_node = this->FindNode(rank_id, arg->result_probe_start_byte);
        ...
        case DPU_FUNC_PACKETWISE_NPHJ_BUILD:
        case DPU_FUNC_PACKETWISE_NPHJ_PROBE:
        case DPU_FUNC_PACKETWISE_LOCAL_HASH_PARTITIONING:
        ...
        case DPU_FUNC_GLB_CHIPWISE_PARTITION_COUNT:
        case DPU_FUNC_GLB_CHIPWISE_PARTITION_PACKET:
        case DPU_FUNC_BG_BROADCAST_ALIGN:
        case DPU_FUNC_BG_BROADCAST_COUNT:
        case DPU_FUNC_LOCAL_HASH_PARTITIONING:
        ...
        const char *buffer_name)
    {
        if (rank_ids.size() == 1)
        {
            int read_size = RetrieveData(rank_ids[0], retreive_buffers[rank_ids[0]], data_bytes[rank_ids[0]], buffer_name);
            return 0;
        }
        /////////////////////////////////////////////////////////////////
    ...
    int IDPHandler::BroadcastDataAligned(
        int rank_id, char* buffers,
        int block_size, const char *buffer_name, int align_byte_size)
    {
        int i = 0;
        struct dpu_set_t dpu;

        // Update Stack Info
        StackNode *node = this->PushStackNodeAligned(rank_id, buffer_name, NULL, block_size, align_byte_size);

        int64_t *buff = (int64_t *)(buffers);
    ...
    int IDPHandler::BroadcastDataAligned(
        ...
        std::vector<int> &block_size, const char *buffer_name, int align_byte_size)
    {
        std::thread *thrs[rank_ids.size()];
        ...
        for (int rank_id : rank_ids)
        {
            thrs[rank_id] = new std::thread(
                ...
                rank_id, std::ref(buffers[rank_id]), block_size[rank_id], align_byte_size, this);
        }
