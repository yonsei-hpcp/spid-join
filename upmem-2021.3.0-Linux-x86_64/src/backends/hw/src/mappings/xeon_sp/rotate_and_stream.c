...
static uint32_t apply_address_translation_on_mram_offset(uint32_t byte_offset)
{
    ...
    const uint32_t mask_21_to_15 = ((1 << (21 - 15 + 1)) - 1) << 15;
    const uint32_t mask_21_to_14 = ((1 << (21 - 14 + 1)) - 1) << 14;
...
static uint64_t fast_address_translation(uint32_t byte_offset)
{
    /* We have observed that, within the 26 address bits of the MRAM address, we need to apply an address translation:
     *
     * virtual[13: 0] = physical[13: 0]
...
static void rank2rank_chipwise_flush_source(
    char* src_rank_base_address,
    int src_rank_idpwise_mram_offset,
    int byte_to_broadcast,
    int bg_count,
    __attribute__((unused)) int num_rank_for_multi_rank_copy)
{
    // if not aligned with 8192 bytes (1MB granularity)
    int word_to_broadcast = byte_to_broadcast / sizeof(int64_t);

    for (int src_bg = 0; src_bg < 8; src_bg++)
    {
        for (int dst_bg = 0; dst_bg < 8; dst_bg++)
        {
            char* curr_src_base_addr = src_rank_base_address + ((src_bg%4) * (256 * 1024)) + ((src_bg>=4) ? 0x40 : 0);
            
            int curr_src_rank_idpwise_mram_offset 
                = src_rank_idpwise_mram_offset + (dst_bg / bg_count) * (byte_to_broadcast);
            
            for (int i = 0; i < word_to_broadcast; i++)
            {
                uint64_t src_offset = fast_address_translation(curr_src_rank_idpwise_mram_offset);
                void* src_host_addr = curr_src_base_addr + src_offset;
                
                __builtin_ia32_clflushopt((uint8_t *)src_host_addr);
                
                // Update mram offset
                curr_src_rank_idpwise_mram_offset += 8;
            }
        }
    }
}

static void rank2rank_chipwise_flush_destination(
    char** dst_rank_base_addresses,
    int dst_rank_idpwise_mram_offset,
    int byte_to_broadcast,
    __attribute__((unused)) int bg_count,
    int num_rank_for_multi_rank_copy)
{
    // if not aligned with 8192 bytes (1MB granularity)

    int word_to_broadcast = byte_to_broadcast / sizeof(int64_t);
    char* curr_dst_base_addresses[NUM_MAX_RANKS];

    for (int src_bg = 0; src_bg < 8; src_bg++)
    {
        for (int dst_bg = 0; dst_bg < 8; dst_bg++)
        {
            for (int r = 0; r < num_rank_for_multi_rank_copy; r++)
            {
                curr_dst_base_addresses[r] = dst_rank_base_addresses[r] + ((dst_bg%4) * (256 * 1024)) + ((dst_bg>=4) ? 0x40 : 0);
            }
            
            int curr_dst_rank_idpwise_mram_offset 
                = dst_rank_idpwise_mram_offset + src_bg * (8 * byte_to_broadcast);

            for (int i = 0; i < word_to_broadcast; i++)
            {
                uint64_t dst_offset0 = fast_address_translation(curr_dst_rank_idpwise_mram_offset);
                uint64_t dst_offset1 = fast_address_translation(curr_dst_rank_idpwise_mram_offset + 8);
                uint64_t dst_offset2 = fast_address_translation(curr_dst_rank_idpwise_mram_offset + 16);
                uint64_t dst_offset3 = fast_address_translation(curr_dst_rank_idpwise_mram_offset + 24);
                uint64_t dst_offset4 = fast_address_translation(curr_dst_rank_idpwise_mram_offset + 32);
                uint64_t dst_offset5 = fast_address_translation(curr_dst_rank_idpwise_mram_offset + 40);
                uint64_t dst_offset6 = fast_address_translation(curr_dst_rank_idpwise_mram_offset + 48);
                uint64_t dst_offset7 = fast_address_translation(curr_dst_rank_idpwise_mram_offset + 56);
                
                for (int r = 0; r < num_rank_for_multi_rank_copy; r++)
                {
                    void* dst_host_addr0 = curr_dst_base_addresses[r] + dst_offset0;
                    __builtin_ia32_clflushopt((uint8_t *)dst_host_addr0);

                    void* dst_host_addr1 = curr_dst_base_addresses[r] + dst_offset1;
                    __builtin_ia32_clflushopt((uint8_t *)dst_host_addr1);
                    
                    void* dst_host_addr2 = curr_dst_base_addresses[r] + dst_offset2;
                    __builtin_ia32_clflushopt((uint8_t *)dst_host_addr2);
                    
                    void* dst_host_addr3 = curr_dst_base_addresses[r] + dst_offset3;
                    __builtin_ia32_clflushopt((uint8_t *)dst_host_addr3);
                    
                    void* dst_host_addr4 = curr_dst_base_addresses[r] + dst_offset4;
                    __builtin_ia32_clflushopt((uint8_t *)dst_host_addr4);
                    
                    void* dst_host_addr5 = curr_dst_base_addresses[r] + dst_offset5;
                    __builtin_ia32_clflushopt((uint8_t *)dst_host_addr5);
                    
                    void* dst_host_addr6 = curr_dst_base_addresses[r] + dst_offset6;
                    __builtin_ia32_clflushopt((uint8_t *)dst_host_addr6);

                    void* dst_host_addr7 = curr_dst_base_addresses[r] + dst_offset7;
                    __builtin_ia32_clflushopt((uint8_t *)dst_host_addr7);
                }

                // Update mram offset
                curr_dst_rank_idpwise_mram_offset += 64;
            }
        }
    }
}
...
static void rank2rank_chipwise_broadcast(
    char* src_rank_base_address,
    char** dst_rank_base_addresses,
    int src_rank_idpwise_mram_offset,
    int dst_rank_idpwise_mram_offset,
    ...
    for (int src_bg = 0; src_bg < 8; src_bg++)
    {
        for (int dst_bg = 0; dst_bg < 8; dst_bg++)
        {
            ...
            for (int i = 0; i < word_to_broadcast; i++)
            {
                uint64_t src_offset = fast_address_translation(curr_src_rank_idpwise_mram_offset);
                void* src_host_addr = curr_src_base_addr + src_offset;
                __m512i src_data_vec_rol0 = _mm512_stream_load_si512((void *)(src_host_addr));
                ...
                {
                    void* dst_host_addr0 = curr_dst_base_addresses[r] + dst_offset0;
                    _mm512_stream_si512((void *)(dst_host_addr0), src_data_vec_rol0);

                    __m512i src_data_vec_rol1 = _mm512_rol_epi64(src_data_vec_rol0, 8);
                    void* dst_host_addr1 = curr_dst_base_addresses[r] + dst_offset1;
                    _mm512_stream_si512((void *)(dst_host_addr1), src_data_vec_rol1);
                    
                    __m512i src_data_vec_rol2 = _mm512_rol_epi64(src_data_vec_rol0, 16);
                    void* dst_host_addr2 = curr_dst_base_addresses[r] + dst_offset2;
                    _mm512_stream_si512((void *)(dst_host_addr2), src_data_vec_rol2);
                    
                    __m512i src_data_vec_rol3 = _mm512_rol_epi64(src_data_vec_rol0, 24);
                    void* dst_host_addr3 = curr_dst_base_addresses[r] + dst_offset3;
                    _mm512_stream_si512((void *)(dst_host_addr3), src_data_vec_rol3);
                    
                    __m512i src_data_vec_rol4 = _mm512_rol_epi64(src_data_vec_rol0, 32);
                    void* dst_host_addr4 = curr_dst_base_addresses[r] + dst_offset4;
                    _mm512_stream_si512((void *)(dst_host_addr4), src_data_vec_rol4);
                    
                    __m512i src_data_vec_rol5 = _mm512_rol_epi64(src_data_vec_rol0, 40);
                    void* dst_host_addr5 = curr_dst_base_addresses[r] + dst_offset5;
                    _mm512_stream_si512((void *)(dst_host_addr5), src_data_vec_rol5);
                    
                    __m512i src_data_vec_rol6 = _mm512_rol_epi64(src_data_vec_rol0, 48);
                    void* dst_host_addr6 = curr_dst_base_addresses[r] + dst_offset6;
                    _mm512_stream_si512((void *)(dst_host_addr6), src_data_vec_rol6);

                    __m512i src_data_vec_rol7 = _mm512_rol_epi64(src_data_vec_rol0, 56);
                    void* dst_host_addr7 = curr_dst_base_addresses[r] + dst_offset7;
                    _mm512_stream_si512((void *)(dst_host_addr7), src_data_vec_rol7);
                }
...
static void xeon_sp_do_chipwise_broadcast(
    RNS_Job_Queue_t *queue, rotate_n_stream_job_t *job, int bg)
{
    ///////////////////////////////////////////////////////
    // Error Check
    ///////////////////////////////////////////////////////
    ...
    // UPMEM MMAPED ADDRESSES
    void *src_rank_base_addr = (void *)(queue->ptr_regions[job->src_rank]);

    // + 1MB. for log only when xfer size does not exceed 63MB - Comment
    size_t temp_src_mram_offset = job->mram_src_offset + (1024 * 1024); 
    size_t temp_dst_mram_offset = job->mram_dst_offset + (1024 * 1024);

    ///////////////////////////////////////////////////////
    // Error Check
    ///////////////////////////////////////////////////////

    int byte_to_broadcast = job->xfer_bytes; // Notice: we just repurpose this variable as byte-to-broadcast

    int num_rank_for_multi_rank_copy = job->num_rank_for_multi_rank_copy;
    char *dst_rank_base_addr[NUM_MAX_RANKS];

    for (int r = 0; r < num_rank_for_multi_rank_copy; r++)
    {
        dst_rank_base_addr[r] = (char *)(queue->ptr_regions[job->rank_ids_for_multi_rank_copy[r]]);
    }
    _mm_mfence();
    rank2rank_chipwise_flush_source(src_rank_base_addr, temp_src_mram_offset, byte_to_broadcast, bg, num_rank_for_multi_rank_copy);
    _mm_mfence();
    rank2rank_chipwise_broadcast(src_rank_base_addr, dst_rank_base_addr, temp_src_mram_offset, temp_dst_mram_offset, byte_to_broadcast, bg, num_rank_for_multi_rank_copy);
    _mm_mfence();
    rank2rank_chipwise_flush_destination(dst_rank_base_addr, temp_dst_mram_offset, byte_to_broadcast, bg, num_rank_for_multi_rank_copy);
    _mm_mfence();


}

static void xeon_sp_do_rnc_thread(
    RNS_Job_Queue_t *queue, rotate_n_stream_job_t *job)
{
...
static void xeon_sp_do_copy_from_pim_opt(RNS_Job_Queue_t *queue, rotate_n_stream_job_t *job)
{
    ...
    //     {
    //         uint64_t host_side_offset = fast_address_translation(temp_src_mram_offset);
...
static void xeon_sp_do_copy_to_pim_opt(RNS_Job_Queue_t *queue, rotate_n_stream_job_t *job)
{
    ...
    for (int p = 0; p < num_packets_per_dpu; p++)
    {
        uint64_t host_side_offset = fast_address_translation(temp_dst_mram_offset);
    ...
    void *queue)
{
    ...
    while (true)
    {
        ...
        break;
        case DPU_TRANSFER_JOB_TYPE_CHIPWISE_BROADCAST_BG8:
        {
            xeon_sp_do_chipwise_broadcast(job_queue, new_job, 1);
        }
        break;
        case DPU_TRANSFER_JOB_TYPE_CHIPWISE_BROADCAST_BG16:
        {
            xeon_sp_do_chipwise_broadcast(job_queue, new_job, 2);
        }
        break;
        case DPU_TRANSFER_JOB_TYPE_CHIPWISE_BROADCAST_BG32:
        {
            xeon_sp_do_chipwise_broadcast(job_queue, new_job, 4);
        }
        break;
        case DPU_TRANSFER_JOB_TYPE_CHIPWISE_BROADCAST_BG64:
        {
            xeon_sp_do_chipwise_broadcast(job_queue, new_job, 8);
        }
        break;
        default:
    ...
    __attribute__((unused)) int num_threads)
{
    ...
    if (direction == true)
    {
        ...
        for (int p = 0; p < num_packets_per_dpu; p++)
        {
            uint64_t host_side_offset = fast_address_translation(temp_mram_address);
            uint8_t *dst_rank_addr_bg = dst_rank_base_addr + host_side_offset;
