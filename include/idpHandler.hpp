...
#define NUM_MAX_RANKS 40
    ...
    // Design choices for different join scenarios

    #define JOIN_DESIGN_JOIN_ALGORITHM_PHJ 0
    #define JOIN_DESIGN_JOIN_ALGORITHM_NPHJ 1
    #define JOIN_DESIGN_JOIN_ALGORITHM_SMJ 2
    #define JOIN_DESIGN_JOIN_ALGORITHM_NL 3
    
    typedef struct
    {
        int64_t input_size = 500000;
        int rank_num = 16;
        int zipf_factor = 0;
        int rs_ratio = 8;
        int rank_set = 1;
        int bank_set = 1;
        bool tpch_example = false;
        int join_alg = 0;
        std::string primary_key;
        std::string foreign_key;
        
        int identical_value_percent = 0; // 100 - skewness
    } join_design_t;
    ...
    public:
        ...
        void SetQueryTree(join_design_t& jd);
        void SendPipelineDoneSignal();
        void AllocateRank(std::vector<int> &rank_ids, int num);
        void AllocateRankPipeline(std::vector<int> &rank_ids, int number_of_ranks, join_design_t join_design);
    ...
    public:
        ...
        static void *ConfigurePacketwiseNPHJBuildArgRank(
            int rank_id, IDPHandler*idp_handler, DPUKernelParams_t &arg_rank,
            int packet_size,
            const char *input_name,
            const char *hash_table_name);

        static void *ConfigurePacketwiseNPHJProbeArgRank(
            int rank_id, IDPHandler*idp_handler, DPUKernelParams_t &arg_rank,
            int packet_size,
            const char *input_name,
            const char *hash_table_name,
        ...
        static void *ConfigureSortMergeJoinLocalPartitioningArgRank(
            int rank_id, IDPHandler*idp_handler, DPUKernelParams_t &arg_rank,
            int elem_range,
            int packet_size,
            int total_idps,
        ...
        static void *ConfigureSortMergeJoinSortProbeArgRank(
            int rank_id, IDPHandler*idp_handler, DPUKernelParams_t &arg_rank,
            int total_idps,
            int64_t partition_num,
            const char *r_sorted_joinkey_name,
            const char *r_local_histgoram_name,
        ...
        static void *ConfigureBGBroadcastingAlignArgRank(
            int rank_id, IDPHandler *idp_handler, DPUKernelParams_t &arg_rank,
            int partition_num,
            RankwiseMemoryBankBuffers_t *imm_hist_buffers,
        ...
        static void *ConfigureBGBroadcastingCountArgRank(
            int rank_id, IDPHandler *idp_handler, DPUKernelParams_t &arg_rank,
            int partition_num,
            const char *input_buffer_name,
            const char *partition_info_name,
